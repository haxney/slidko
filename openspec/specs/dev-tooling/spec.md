...
TBD — captures the developer-facing tooling gate for Slidko: type checking,
exact dependency pinning, a unified verification entry point, an
offline-capable pre-commit gate, and firmware formatting enforcement, so
local, overnight, and CI verification cannot diverge.
