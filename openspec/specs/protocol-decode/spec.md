...
Backend-abstracted protocol decoding: Measure infers what a signal is and its
parameters, Decode turns it into events. Wraps the sigrok decoder corpus
(~150 protocol decoders) behind a normalized, backend-agnostic event schema
rather than rewriting decoders — fed entirely by Measure's inference, with
