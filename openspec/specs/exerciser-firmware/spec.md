...
RP2040/RP2350 exerciser firmware (pico-sdk, C, CMake): a JSON-lines
command protocol over USB CDC for stimulus generation and read-only bus
interrogation, clock-parameterized so the 133→150 MHz migration is a
config change, silicon-aware around the E9 erratum, and headless-CI
buildable from a self-provisioning pinned SDK. The DUT-control boundary
is absolute — no flashing, no MSP writes, no commanding user firmware.
