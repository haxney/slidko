...
Closed-list protocol classification and per-protocol parameter inference —
"measure it, don't ask" — over the drone/maker bench protocol universe:
UART (+ SBUS/CRSF/SmartAudio/MSP costumes), I²C, SPI, WS2812, PWM/servo,
DShot, CAN (recognition), analog video (recognition only). Hand-derived
discriminators over interval/autocorrelation features; no ML, zero manually
supplied parameters, and every claim carries a numeric confidence.
