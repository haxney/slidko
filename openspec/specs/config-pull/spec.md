...
Read-only DUT configuration interrogation over documented protocols
(e.g., Betaflight MSP query, CLI dump capture). Feeds Diagnose's fault
tree as a probe-free first layer — config causes are checked and cited
before any physical probe instruction is generated. No write, flash, or
command capability exists in this module, by product-premise boundary.
