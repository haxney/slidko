...
from src.slidko.narrate.model import Assertion, Evidence
...
def detect_coincidences(events: List[DecodedEvent], findings: List[SmokeFinding], samplerate_hz: int) -> List[Assertion]:
    """
        ...
        if event.kind == "i2c.nak":  # Only look at NAK events
            for finding in findings:
                ...
                if sample_delta <= COINCIDENCE_WINDOW_SAMPLES:
                    ...
                    evidence_indices = tuple([event.sample, finding.sample])
                    ...
                    text = (f"Coincidence detected between NAK on channel {event.channel} "
