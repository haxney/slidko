...
def is_pad_level_claim(instruction: Instruction) -> bool:
    ...
    A pad-level placement claim places a probe/clip on a specific physical point
    identified by pad/pin/test-point (e.g. action="clip", target="TP7" or target="U3 pin 4").
...
def validate_instruction(instruction: Instruction, retrieval) -> list[ValidationError]:
    """
    ...
    # Rule 1: expected_outcome_per_hypothesis must be a non-empty dict (it's always required by dataclass)
    # But let's make sure it exists and is valid
    if instruction.expected_outcome_per_hypothesis is None:
        errors.append(ValidationError("Missing required field: expected_outcome_per_hypothesis"))
    ...
    if is_pad_level_claim(instruction):
        ...
        if not instruction.unknown and not instruction.citations:
            errors.append(ValidationError("Pad-level placement claim requires citation or unknown=True"))
    ...
    if is_pad_level_claim(instruction) and not instruction.hazard_notes:
        errors.append(ValidationError("Pad-level placement claim requires non-empty hazard_notes"))
    ...
    # Accessibility filter (Rule 4 from design) - check for fine pitch IC pins on powered boards
    MIN_PITCH_FOR_ACCESSIBILITY = 0.5  # The minimum pitch that triggers accessibility checks
    
    if is_pad_level_claim(instruction):
        power_state = instruction.parameters.get("power_state", "off").lower()
        pitch = instruction.parameters.get("pitch", None)
        
        # Check if the target seems to be an IC pin and has fine pitch and is on a powered board
        if (_is_ic_pin(instruction.target) and 
            power_state == "on" and 
            pitch is not None and 
            pitch < MIN_PITCH_FOR_ACCESSIBILITY):
            
            errors.append(ValidationError(f"Accessibility filter: Fine pitch IC pin ({pitch}mm) on powered board not allowed"))
